...
\]
We model in \emph{price} coordinates and read off implied vols at the end;
the price$\leftrightarrow$vol transform and the reason for this choice are
on slide 5.
...
\item \emph{Dimension.} $N$ is large; $\cC_{\mathrm{NA}}$ lives in $\R^N$.
  ...
  redundant.
\item \emph{Coordinates matter.} This clean linear/convex description holds
  in \emph{price} space; in implied-vol space the same region is non-convex
  (more on slide 5).
...
% ============================ SLIDE 4 ============================
\slide{Prices vs.\ implied vol --- why we model prices}

\textbf{The two coordinate systems.} A European call has price $C$ and an
equivalent \emph{Black--Scholes implied volatility} $\sigma$ --- the single
vol input that makes the BS formula reproduce the market price:
\[
C \;=\; C_{\mathrm{BS}}(\sigma;\,\tau,m), \qquad
\frac{\partial C_{\mathrm{BS}}}{\partial\sigma}=\text{vega}>0 .
\]
Vega $>0$ $\Rightarrow$ for each $(\tau,m)$ the map $\sigma\mapsto C$ is
strictly increasing, hence \emph{invertible}. Normalised price
$c=C/S$ and $\sigma$ carry the \emph{same information}; one is a smooth,
monotone, entrywise reparametrisation of the other.

\vspace{6pt}
\textbf{So why not model $\sigma$ directly?} Because no-arbitrage is ugly in
vol coordinates:
\begin{itemize}
\item In \emph{price} space, no static arbitrage $=$ \emph{linear}
  inequalities $A\mathbf c\ge\mathbf{\hat b}$ (monotonicity, convexity) ---
  a convex polyhedron.
\item In \emph{vol} space, the butterfly constraint becomes the
  \emph{nonlinear} Durrleman condition; the arbitrage-free region is
  non-convex. Linearity and convexity are lost.
\end{itemize}
Every downstream step --- the factor polytope, the linear pullback, the
convex geometry the $h$-transform conditioning relies on --- needs the
\emph{linear} price-space description.

\vspace{6pt}
\textbf{Where the transform sits in the pipeline.}
\[
\xi \;\xrightarrow[\text{linear}]{\;\mathbf c=\mathbf G_0+\mathbf G^\top\xi\;}\;
\mathbf c \;\xrightarrow[\text{entrywise, monotone}]{\;\sigma_j=C_{\mathrm{BS}}^{-1}(c_j;\tau_j,m_j)\;}\;
\sigma \;=\; \text{implied-vol surface.}
\]
We model the \emph{factor} $\xi$; arbitrage-free prices are linear in $\xi$;
the \emph{final} step inverts BS pointwise (1-D root-find per node, vega
$>0$ guarantees a unique root) to deliver the object desks actually quote
--- the vol surface.

\vspace{4pt}
\textbf{Consequence.} A forecast that is arbitrage-free in price space maps
to an arbitrage-free implied-vol surface automatically --- no extra
constraints needed in vol coordinates.

\footer{Model prices (linear, convex no-arbitrage); report vol (what the desk wants). The bridge is BS inversion.}
\clearpage
...
\begin{enumerate}
\item Observe today's surface; project prices to the factor
  $\xi_k=\mathbf G^{\top+}(\mathbf c_k-\mathbf G_0)$, compute context $Y_k$.
...
\item Invert Black--Scholes entrywise, $\sigma_j=C_{\mathrm{BS}}^{-1}(c_j)$
  $\Rightarrow$ implied-vol surface (the quoted object).
\end{enumerate}
